...
describe 'compiled component amazonmq' do
  ...
  context 'cftest' do
    it 'compiles test' do
      expect(system("cfhighlander cftest #{@validate} --tests tests/maintenance_window.test.yaml")).to be_truthy
    end      
  end
  
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/maintenance_window/amazonmq.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "Broker" do
      ...
      it "to have property MaintenanceWindowStartTime" do
          expect(resource["Properties"]["MaintenanceWindowStartTime"]).to eq({"DayOfWeek"=>"Sunday", "TimeOfDay"=>"01:00", "TimeZone"=>"Australia/Melbourne"})
      end
