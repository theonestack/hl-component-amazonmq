...
describe 'compiled component amazonmq' do
  ...
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/tags/amazonmq.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "SecurityGroupAmazonmq" do
      ...
      it "to have property Tags" do
          expect(resource["Properties"]["Tags"]).to eq([{"Key"=>"Environment", "Value"=>{"Ref"=>"EnvironmentName"}}, {"Key"=>"EnvironmentType", "Value"=>{"Ref"=>"EnvironmentType"}}, {"Key"=>"Name", "Value"=>{"Fn::Sub"=>"${EnvironmentName}-AmazonMQ"}}, {"Key"=>"CostCenter", "Value"=>{"Fn::Sub"=>"${CostCenter}"}}])
      end
