...
describe 'compiled component amazonmq' do
  ...
  context 'cftest' do
    it 'compiles test' do
      expect(system("cfhighlander cftest #{@validate} --tests tests/amq_xml.test.yaml")).to be_truthy
    end      
  end
  
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/amq_xml/amazonmq.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "Config" do
      let(:resource) { template["Resources"]["Config"] }

      it "is of type AWS::AmazonMQ::Configuration" do
          expect(resource["Type"]).to eq("AWS::AmazonMQ::Configuration")
      end
      
      it "to have property Name" do
          expect(resource["Properties"]["Name"]).to eq({"Fn::Sub"=>"${EnvironmentName}-AmazonMQ"})
      end
      
      it "to have property EngineType" do
          expect(resource["Properties"]["EngineType"]).to eq("5.15.6")
      end
      
      it "to have property EngineVersion" do
          expect(resource["Properties"]["EngineVersion"]).to eq("ACTIVEMQ")
      end
      
      it "to have property Data" do
          expect(resource["Properties"]["Data"]).to eq({"Fn::Base64"=>"<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>\n  <broker xmlns=\"http://activemq.apache.org/schema/core\" start=\"false\">\n    <destinationPolicy>\n      <policyMap>\n        <policyEntries>\n          <policyEntry topic=\">\">\n            <pendingMessageLimitStrategy>\n              <constantPendingMessageLimitStrategy limit=\"3000\"/>\n            </pendingMessageLimitStrategy>\n          </policyEntry>\n        </policyEntries>\n      </policyMap>\n    </destinationPolicy>\n    <plugins>\n    </plugins>\n  </broker>\n"})
      end
    ...
    context "Broker" do
      ...
      it "to have property Configuration" do
          expect(resource["Properties"]["Configuration"]).to eq({"Id"=>{"Fn::GetAtt"=>["Config", "Id"]}, "Revision"=>{"Fn::GetAtt"=>["Config", "Revision"]}})
      end
      ...
      it "to have property Users" do
          expect(resource["Properties"]["Users"]).to eq([{"Password"=>{"Fn::GetAtt"=>["administratorSSMSecureParameter", "Password"]}, "Username"=>"administrator", "ConsoleAccess"=>true}])
      end
