...
describe 'compiled component amazonmq' do
  ...
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/security_groups/amazonmq.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "SecurityGroupAmazonmq" do
      ...
      it "to have property SecurityGroupIngress" do
          expect(resource["Properties"]["SecurityGroupIngress"]).to eq([{"FromPort"=>8162, "IpProtocol"=>"TCP", "ToPort"=>8162, "Description"=>{"Fn::Sub"=>"internal access to web console"}, "CidrIp"=>{"Fn::Sub"=>"10.0.0.0/8"}}, {"FromPort"=>61617, "IpProtocol"=>"TCP", "ToPort"=>61617, "Description"=>{"Fn::Sub"=>"access to OpenWire amq endpoint from local ips"}, "CidrIp"=>{"Fn::Sub"=>"10.8.10.6/32"}}, {"FromPort"=>61617, "IpProtocol"=>"TCP", "ToPort"=>61617, "Description"=>{"Fn::Sub"=>"access to OpenWire amq endpoint from local ips"}, "CidrIp"=>{"Fn::Sub"=>"10.6.5.12/32"}}, {"FromPort"=>8883, "IpProtocol"=>"tcp", "ToPort"=>8883, "Description"=>{"Fn::Sub"=>"MQTT access from another security group"}, "SourceSecurityGroupId"=>{"Fn::Sub"=>"sg-fqerekjrhr"}}])
      end
