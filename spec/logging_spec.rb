...
describe 'compiled component amazonmq' do
  ...
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/logging/amazonmq.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "Broker" do
      ...
      it "to have property Logs" do
          expect(resource["Properties"]["Logs"]).to eq({"Audit"=>true, "General"=>true})
      end
