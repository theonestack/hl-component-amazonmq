...
describe 'compiled component amazonmq' do
  ...
  context 'cftest' do
    it 'compiles test' do
      expect(system("cfhighlander cftest #{@validate} --tests tests/engine_version.test.yaml")).to be_truthy
    end      
  end
  
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/engine_version/amazonmq.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "Broker" do
      ...
      it "to have property EngineVersion" do
          expect(resource["Properties"]["EngineVersion"]).to eq("5.16.4")
      end
